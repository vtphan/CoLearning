...
def student_views_discussions():
    ...
    if 'student' not in groups.get(user_id):
        redirect(URL('not_authorized'))
    rows = db(db.discussion.student_id==user_id).select(orderby=~db.discussion.posted_at)
    return dict(rows=rows)


@action('teacher_views_discussion/<discussion_id>', method='GET')
@action.uses(auth.user, 'teacher_views_discussion.html')
def teacher_views_discussions(discussion_id):
    user_id = auth.get_user()['id']
    if 'teacher' not in groups.get(user_id) and 'ta' not in groups.get(user_id):
        redirect(URL('not_authorized'))
    discussion = db.discussion[discussion_id]
    return dict(d=discussion, user_id=user_id)


@action('student_views_discussion/<discussion_id>', method='GET')
@action.uses(auth.user, 'student_views_discussion.html')
def student_views_discussions(discussion_id):
    user_id = auth.get_user()['id']
    if 'student' not in groups.get(user_id):
        redirect(URL('not_authorized'))
    discussion = db.discussion[discussion_id]
    if user_id != discussion.student_id:
        redirect(URL("not_authorized"))
    # print(user_id, discussion.student_id)
    return dict(d=discussion, user_id=user_id)